...
def get_top_level(
    title: str, size: str = "300x120", bg: str = "#ffffff"
) -> tk.Toplevel:
...
def __add_entry(
    frame: tk.Toplevel, text_var: tk.StringVar, text: str, label: str, width: int = 16
) -> tk.Entry:
...
def __download_local():
    ...
    if is_same_network(my_ip, LOCAL_IP):
        print("Using local IP for SCP.")
        ip = LOCAL_IP
    else:
        print("Using Tailscale IP for SCP.")
        ip = EXTERNAL_IP

    cmd = f"scp -P {str(LOCAL_PORT)} -o StrictHostKeyChecking=no admin@{ip}:/home/admin/Documents/5400_data.db {DEFAULT_FILE_NAME}"
    output_lines = []
    ...
    with subprocess.Popen(
        ...
        universal_newlines=True,  # Safe fallback for older Python versions
    ) as p:
    ...
    # Optional: check if process succeeded
    if return_code != 0:
        print("Command failed!")
        print(output)  # Or log/raise error

    # if return_code or len(output) == 0:
    ...
    # ):
    #     dl_label.config(
    #         text="Download Failed: Wrong PW in file", fg=__get_color("failure")
    #     )
    # else:
    print(output)
...
edit_menu.add_command(label="Download Data", command=download_data_locally)
...
# Control Area
data_plot_type = 0
update_button = __add_button(
    frame, "Update Graph", cmd=update_graph, height=2, width=12, side="left"
)
# smooth_button = __add_button(frame, "Smooth Data", cmd=smooth_data, height=2, width=12, side="left")
volumt_button = __add_button(
    frame, "Volume", cmd=switch_data, height=2, width=12, side="left"
)
